...
class LfoModule(ConnectionProtocol, PatchProtocol, BaseModule):
    def __init__(self, mod_id: str, parent_root: tk.Tk = None):
        super().__init__(mod_id, "lfo")  # FIRST — creates self.mcast_group

        self.inputs = {}
        self.outputs = {"cv": {"type": "cv", "group": self.mcast_group}}  # now safe
    ...
    def _setup_gui(self, parent_root):
        ...
        self.gui_leds["cv"] = JackWidget(self.root, "cv", "CV Output", short_press_callback=self.initiate_connect, verbose_text=False)
        self.gui_leds["cv"].pack(pady=5)
