...
class PatchProtocol:
    ...
    def get_capabilities(self) -> Dict:
        return {
            "name": self.module_id,
            "type": self.type,
            "controls": [{"id": k, "range": v, "default": self.controls.get(k, v[0])} for k, v in self.control_ranges.items()],
    ...
    def restore_patch(self, data: bytes or Dict):
        ...
        with self.lock:
            self.controls.update({k: max(self.control_ranges[k][0], min(self.control_ranges[k][1], v)) for k, v in data.get("controls", {}).items() if k in self.control_ranges})
            ...
            self.led_states.update({io: LedState.SOLID for io in self.outputs})
